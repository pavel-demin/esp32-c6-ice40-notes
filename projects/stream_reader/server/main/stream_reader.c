...
static int sock_recv(int sock, void *buffer, size_t size)
{
  size_t offset, limit;
  offset = 0;
  limit = size;
...
static int sock_send(int sock, void *buffer, size_t size)
{
  size_t offset, limit;
  offset = 0;
  limit = size;
...
static void tcp_server_task(void *pvParameters)
{
  ...
  while(1)
  {
    ...
    while(1)
    {
      ...
      if(fds.revents & POLLERR) break;

      /* read 2 KB if ready */
      if(gpio_get_level(PIN_NUM_FPGA_DAT1))
      {
        size = 2048;
        command = 511 << 20 | 2 << 17;
        gpio_set_level(PIN_NUM_CS, 0);
