...
(
  ...
  reg [4:0] int_cntr_reg = 5'd0;

  wire [31:0] int_tx_tdata_wire [1:0];
  wire int_tx_tvalid_wire, int_tx_tready_wire;

  wire [31:0] int_rx_tdata_wire [1:0];
  ...
  wire int_reset_wire, int_ready_wire;

  assign int_ready_wire = int_cntr_reg == 5'd20;

  always @(posedge spi_sclk)
  begin
    if(int_rx_tvalid_wire & ~int_rx_tdata_wire[1][30])
    begin
      int_cntr_reg <= 5'd0;
    end
  ...
  axis_fifo_async #(
    ...
    .m_axis_aclk(spi_sclk),
    .m_axis_aresetn(1'b1),

    .m_axis_tdata(int_tx_tdata_wire[0]),
  ...
  bit_reorderer #(
    .DATA_WIDTH(32)
  ) br_tx (.in_data(int_tx_tdata_wire[0]), .out_data(int_tx_tdata_wire[1]));
  ...
  axis_upsizer #(
    ...
    .aclk(spi_sclk),
    .aresetn(~int_reset_wire),

    .cfg_data(16'd31),

    .s_axis_tdata(spi_mosi),
    .s_axis_tvalid(~spi_ssel),

    .m_axis_tdata(int_rx_tdata_wire[0]),
  ...
  bit_reorderer #(
    .DATA_WIDTH(32)
  ) br_rx (.in_data(int_rx_tdata_wire[0]), .out_data(int_rx_tdata_wire[1]));

  axis_fifo_async #(
    .AXIS_TDATA_WIDTH(32),
    .ADDR_WIDTH(4)
  ) fifo_rx (
    .s_axis_aclk(spi_sclk),
    .s_axis_aresetn(1'b1),

    .s_axis_tdata(int_rx_tdata_wire[1]),
